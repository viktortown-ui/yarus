...
scripts=[(ROOT/'web'/n).read_text(encoding='utf-8') for n in ['domain.js', 'vendor/qr-encode.js', 'vendor/qr-decode.js', 'vendor/code128-patterns.js', 'codes.js', 'scanner.js', 'app.js']]
...
for name,code in zip(['domain.js', 'vendor/qr-encode.js', 'vendor/qr-decode.js', 'vendor/code128-patterns.js', 'codes.js', 'scanner.js', 'app.js'],scripts):html=html.replace(f'<script src="{name}"></script>','<script>'+code+'</script>')
