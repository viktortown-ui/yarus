...
func TestMigratePreviousDataCopiesEncryptedStoreAndProfile(t *testing.T) {
	base := t.TempDir()
	root := filepath.Join(base, "YARUS-1.1.2-READY")
	target := filepath.Join(root, "01-Windows", "YARUS-1.1.2-Windows-x64")
	oldInstall := filepath.Join(base, "YARUS-1.1.1-READY", "01-Windows", "YARUS-1.1.1-Windows-x64")
	...
	migrated, err := migratePreviousData(root, target)
	if err != nil || !migrated {
	...
	if err != nil || again {
		t.Fatalf("second migration must not overwrite data: migrated=%v err=%v", again, err)
	}
}
