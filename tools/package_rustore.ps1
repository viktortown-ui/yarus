...
$zip = Join-Path $dist 'YARUS-1.2.0-RuStore-submission.zip'

New-Item -ItemType Directory -Force -Path $target | Out-Null
foreach ($required in @(
    (Join-Path $dist 'android\YARUS-1.2.0-RuStore.apk'),
    (Join-Path $dist 'android\YARUS-1.2.0-RuStore.aab'),
...
Copy-Item -LiteralPath (Join-Path $dist 'android\YARUS-1.2.0-RuStore.apk'),(Join-Path $dist 'android\YARUS-1.2.0-RuStore.aab'),(Join-Path $dist 'android\SHA256SUMS.txt') -Destination $binaries -Force
...
Copy-Item -LiteralPath (Join-Path $root 'docs\ИНСТРУКЦИЯ-1.2.0.html') -Destination (Join-Path $target 'ИНСТРУКЦИЯ.html') -Force
