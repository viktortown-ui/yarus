...
global.YarusDomain=require('../web/domain.js');let stored='';
...
test('Android host owns state, grants role-scoped access and retires atomically',async()=>{
 ...
 const transfer=await H.transferState('Новый компьютер');assert.equal(transfer.state.host.status,'active');assert.equal(transfer.state.host.epoch,2);assert.equal(H.info().host.status,'retired');
 await assert.rejects(()=>H.request('/api/command',{method:'POST',authorization:auth,body:{...movement,id:'command-0004'}}),/больше не является главным/);
});
